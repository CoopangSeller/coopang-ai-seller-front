...
import { ThumbnailConfig, ModelType } from '../../types';
...
const ThumbnailGenerator: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      const prompt = `A professional ecommerce thumbnail for ${config.productName}. 
      Style: ${config.style}. ${config.features}. 
      ${config.hasPerson ? 'Including a professional human model showcasing the product.' : 'Product focused shot.'}
      Clean commercial look, studio lighting.`;
      
      const imageUrl = await generateImage(prompt, modelType, "1:1", config.referenceImage);
    ...
    <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12">
      <div className="bg-white p-8 rounded-2xl shadow-xl border border-slate-100 space-y-6 h-fit">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700">강조하고 싶은 특징</label>
            <input 
              className="w-full px-4 py-3 rounded-lg border border-slate-200 outline-none focus:ring-2 focus:ring-blue-500"
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700">스타일 선택</label>
            <div className="grid grid-cols-3 gap-2">
              {(['Clean', 'Lifestyle', 'Creative'] as const).map(s => (
                <button
                  key={s}
                  onClick={() => setConfig({ ...config, style: s })}
                  className={`py-2 rounded-lg text-sm font-medium border transition-all ${config.style === s ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-50 text-slate-600 border-slate-200'}`}
                >
                  {s === 'Clean' ? '깔끔한' : s === 'Lifestyle' ? '라이프스타일' : '창의적인'}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 py-2">
            <input 
              ...
              onChange={e => setConfig({ ...config, hasPerson: e.target.checked })}
              className="w-5 h-5 rounded text-blue-600" 
            />
            <label htmlFor="person" className="text-sm font-medium text-slate-700">인물 모델 포함 여부</label>
          </div>

          <div className="space-y-2 pt-2 border-t border-slate-100">
            <label className="text-sm font-semibold text-slate-700">모델 등급 선택</label>
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" checked={modelType === ModelType.FREE} onChange={() => setModelType(ModelType.FREE)} className="text-blue-600" />
                <span className="text-sm">무료 (Flash)</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" checked={modelType === ModelType.PAID} onChange={() => setModelType(ModelType.PAID)} className="text-blue-600" />
                <span className="text-sm font-bold text-blue-700">유료 (Nanobanana Pro)</span>
              </label>
            </div>
          </div>

          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700">제품 원본 사진 업로드 (선택)</label>
            <input type="file" onChange={handleFileChange} className="text-xs text-slate-500 w-full" />
          </div>
        </div>
      ...
      <div className="flex flex-col items-center justify-center space-y-6">
        <div className="relative w-full max-w-[500px] aspect-square bg-white rounded-3xl shadow-2xl overflow-hidden border-8 border-white flex items-center justify-center">
          {resultImage ? (
            <img src={resultImage} alt="Generated Thumbnail" className="w-full h-full object-cover" />
          ) : (
            <div className="flex flex-col items-center text-slate-300">
              ...
              <p className="font-medium text-slate-400">설정 후 생성을 눌러주세요</p>
            </div>
          )}
